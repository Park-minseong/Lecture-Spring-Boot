...
//실제 서버에 파일업로드 처리를 해주는 클래스
public class FileUtils {
	public List<BoardFile> parseFileInfo(int boardSeq, HttpServletRequest request, MultipartHttpServletRequest multipartServletRequest) throws IOException{
		List<BoardFile> fileList = new ArrayList<BoardFile>();
		
		//서버의 루트경로 가져오기
		String rootPath = request.getSession().getServletContext().getRealPath("/");
		System.out.println(request.getSession()+"?????"+request.getSession().getServletContext()+"/////////"+request.getSession().getServletContext().getRealPath("/"));
		...
		while(iterator.hasNext()) {
			//iterator 에 담겨있는 파일이름들로 첨부파일 꺼내오기
